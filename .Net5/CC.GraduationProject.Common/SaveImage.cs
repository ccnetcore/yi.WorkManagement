...
namespace CC.GraduationProject.Common
{
    ...
    public static class SaveImage
    {
        ...
        public static bool ByStringToSave(string name, string iss)
        {
            ...
            string StudentWorkImagess = "StudentWorkImages" ;

            if (Directory.Exists(@"./wwwroot/"+StudentWorkImagess) == false)//如果不存在就创建file文件夹
            {
              Directory.CreateDirectory(@"./wwwroot/" + StudentWorkImagess);
            }



            bmp.Save(@"./wwwroot/"+ StudentWorkImagess + "/" + name + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
            ms.Close();
            return true;
        }
